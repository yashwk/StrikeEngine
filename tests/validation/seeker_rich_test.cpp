...
VehicleInitState makeInit(double x, double y, double z, Allegiance alleg)
{
...
// Entity signature metadata (type, profile ids, EIRP) lives on the
// VehicleConfig; this builds the standard static-geometry config with them.
VehicleConfig targetConfig(EntityType type,
                           ...
                           double eirp = 0.0)
{
    ...
    cfg.type = type;
    cfg.rcsProfileId = rcs;
    cfg.irProfileId = ir;
    cfg.emitterEirpW = eirp;
    return cfg;
}
...
int main()
{
    ...
    // ---- 1. PassiveRF ----
    {
        ...
        const auto seekerId = addVehicle(kernel,
            makeInit(0.0, 0.0, 10000.0, Allegiance::Friendly), cfg);

        // Emitter at 100 km: SNR ~57.6 dB -> detected.
        const auto emitterFar = addVehicle(kernel,
            makeInit(1.0e5, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Aircraft, "", "", 1.0e6));
        kernel.step(dt);
    ...
    }
    {
        ...
        addVehicle(kernel,
            makeInit(4.0e7, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Aircraft, "", "", 1.0e6));
        kernel.step(dt);
        check(!kernel.getSeekers().isLocked[seekerId],
    ...
    }
    {
        ...
        addVehicle(kernel,
            makeInit(5.0e4, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Aircraft, "", "", 0.0));
        kernel.step(dt);
        check(!kernel.getSeekers().isLocked[seekerId],
    ...
    // ---- 2. SARH (bistatic) ----
    {
        ...
        addVehicle(kernel, makeInit(1000.0, 0.0, 10000.0, Allegiance::Friendly), rfCfg);
        ...
        const auto sarhNearId = addVehicle(kernel,
            makeInit(1000.0, 0.0, 10000.0, Allegiance::Friendly), sarhNear);
        ...
        const auto sarhFarId = addVehicle(kernel,
            makeInit(1000.0, 0.0, 10000.0, Allegiance::Friendly), sarhFar);

        addVehicle(kernel,
            makeInit(2010.0, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Aircraft, kRcsFlat));
        kernel.step(dt);
    ...
    // ---- 3. IR Beer-Lambert transmittance ----
    {
        ...
        const auto clearId = addVehicle(kernel,
            makeInit(0.0, 0.0, 10000.0, Allegiance::Friendly), clearCfg);
        ...
        const auto hazyId = addVehicle(kernel,
            makeInit(0.0, 0.0, 10000.0, Allegiance::Friendly), hazyCfg);

        addVehicle(kernel,
            makeInit(60000.0, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Aircraft, "", kIrFlat));
        kernel.step(dt);
    ...
    }
    {
        ...
        addVehicle(kernel,
            makeInit(40000.0, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Aircraft, "", kIrFlat));
        kernel.step(dt);
        check(kernel.getSeekers().isLocked[clearId] &&
    ...
    // ---- 4. Chaff / flare decoys (strongest signal seduces the seeker) ----
    {
        ...
        rfCfg.seeker.type = SeekerType::RF;
        const auto seekerId = addVehicle(kernel,
            makeInit(0.0, 0.0, 10000.0, Allegiance::Friendly), rfCfg);
        const auto realTarget = addVehicle(kernel,
            makeInit(100.0, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Aircraft, kRcsFlat));
        const auto chaffId = addVehicle(kernel,
            makeInit(50.0, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Chaff, kRcsLarge));
        kernel.step(dt);
        check(kernel.getSeekers().isLocked[seekerId] &&
    ...
    }
    {
        ...
        const auto seekerId = addVehicle(kernel,
            makeInit(0.0, 0.0, 10000.0, Allegiance::Friendly), irCfg);
        addVehicle(kernel,
            makeInit(100.0, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Aircraft, "", kIrFlat));
        const auto flareId = addVehicle(kernel,
            makeInit(50.0, 0.0, 10000.0, Allegiance::Hostile),
            targetConfig(EntityType::Flare, "", kIrHot));
        kernel.step(dt);
        check(kernel.getSeekers().isLocked[seekerId] &&
    ...
    // ---- 5. Public SeekerConfig round-trip ----
    {
        ...
        VehicleInitState init = makeInit(0.0, 0.0, 10000.0, Allegiance::Friendly);
        ...
        check(kernel.getStatus().emitterEirpW[id] == 2.5e6,
              "VehicleConfig.emitterEirpW round-trips into statusBlock");
    }
