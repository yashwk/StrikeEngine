...
- `EnvironmentConfig.hpp`: local terrain/wind callbacks, optional geodetic
  `TerrainSource` database; earth options (`useEcefTruth`, gravity, Coriolis,
...
- `GlobalTerrain.hpp/.cpp`: dependency-free geodetic raster database with
  nearest/bilinear interpolation, explicit NODATA/coverage status, WGS84-scaled
  surface normals/slope, dateline normalization, and ESRI ASCII Grid loading.
  Optional GDAL loading normalizes CRS/geotransform-aware GeoTIFF/COG, DTED, VRT,
  and other single-band sources to WGS84; `TerrainTileCache` bounds normalized
  source retention with a thread-safe LRU.
- `EventSystem.cpp`: local terrain views, geodetic altitude, ellipsoid impact
  clamp, global-geodetic terrain queries in local/ECEF truth, terrain surface
  data on ground-impact events, failure event vocabulary (`MotorFailure`/`EngineFailure`/`TankFailure`/
...
| `global_terrain` | nearest/bilinear raster sampling, coverage/NODATA status, surface normal/slope, ESRI ASCII and optional GDAL loading, LRU reuse, dateline normalization, local/ECEF terrain impact and event payload |
...
- **Physics/environment:** CFD-validated coefficient data, `AeroForces`,
  ...
  atmosphere/weather, automatic spatial multi-tile terrain discovery/streaming,
  tile prefetch, datum/geoid, and higher-fidelity polar coverage. Optional GDAL
  GeoTIFF/DTED/VRT ingestion and bounded normalized-source caching are MVPs.
...
telemetry (binary reader done); global terrain automatic multi-tile discovery/
streaming, tile prefetch, datum/geoid, and higher-fidelity polar coverage;
