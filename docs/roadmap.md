...
* **Tasks — [~] PARTIAL:** WGS84 Gravity, Earth Rotation, Coriolis effects, Adaptive `RK45Integrator`, and the `GPUBackend` (Vulkan/CUDA). Adaptive RK45, an optional Vulkan backend, WGS84 normal and spherical point-mass gravity, local ENU/NED frame transforms, local Coriolis, centrifugal effects, and local moving-origin transport are implemented; global ECEF propagation and CUDA remain pending.
...
The current engine checkpoint has **17/17 CTest tests passing**. W1–W12 MVP
...
effects in W10, and spherical gravity in W12. Remaining roadmap items are
global ECEF state propagation, transport beyond the local reference frame, expanded
